...
from schema import(
    PostResponse,UserCreate,UserUpdate,
    UserPublic, UserPrivate, Token,
    PaginatedPostsResponse,ChangePasswordRequest, ResetPasswordRequest, ForgotPasswordRequest)
...
async def forgot_password(
    ...
    result = await db.execute(
        select(model.User).where(
            func.lower(model.User.email) == request_data.email.lower(),
        ),
    )
    user = result.scalars().first()
    ...
    if user:
        ...
        expires_at = datetime.now(UTC) + timedelta(
            minutes=setting.reset_token_expire_minutes,
        )
...
async def reset_password(
    ...
    result = await db.execute(
        select(model.User).where(model.User.id == reset_token.user_id),
    )
    user = result.scalars().first()
...
async def change_password(
    ...
    await db.execute(
        sql_delete(model.PasswordResetToken).where(
            model.PasswordResetToken.user_id == current_user.id,
        ),
    )
...
async def update_user(user_id: int, user_update: UserUpdate, current_user:CurrentUser, db: Annotated[AsyncSession,Depends(get_db)]):
    ...
    if user_update.email is not None and user_update.email.lower() != user.email.lower():
        ...
        if existing_user:
            raise HTTPException(
                status_code= status.HTTP_400_BAD_REQUEST, detail="email already exist with another account"
            )

    # this is manual check the other version of setattr using key value pair
...
async def upload_profile_picture(user_id: int,
    ...
    if current_user.id != user_id:
        raise HTTPException(
            ...
            detail="Not Authorized too update this user picture"
        )

    content = await file.read()

    if len(content) > setting.max_upload_size_bytes:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail= f"File to large. Maximum size is {setting.max_upload_size_bytes / (1024 * 1024)} MB."
        )
...
async def user_delete(user_id: int, current_user: CurrentUser, db: Annotated[AsyncSession, Depends(get_db)]):
